...
class CCAllBranchController extends Controller
{
    ...
    public function allfac()
    {
        ...
        //     $masterLine = MasterLine::with('ltarget')->where('branch', $value->kode_branch)->where('branch_detail', $value->kodebranch)->get();
            
        //     // foreach ($masterLine as $key2 => $value2) {
        //     //     for
        //     // }
        // }
        return view('CommandCenter.allfac_qc', compact('dataRework', 'dataBranch'));
    }
    ...
    public function qc(Request $request)
    {
       ...
       //  dd($tanggal);
        ...
        if(LineDetail::where('tgl_pengerjaan', $tanggal)->count()) {
            $dataRework =   $this->dataRework($tanggal);
        }else{
            $dataRework = 0;
        }
       ...
       if ($jumlah == 0) {
        ...
        return view('CommandCenter.allqc', compact('dataSemua', 'dataRework', 'dataRejectCutting', 'dataRejectGarment',
                                         'dataFactoryAudit', 'dataFinalInspection', 'dataSampleInspection', 'dataKlaimBuyer'));
    }

    public function dataRework($tanggal)
    {
        // BUAT GRAFIK 
         $data =  MasterLine::all();
        ...
        foreach ($data as $key => $value) {
            foreach ($detail as $key => $value2) {
                if ($value->id == $value2->id_line) {
                    ...
                    $terpenuhi = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('target_terpenuhi');
                    $total_reject = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('total_reject');
    ...
    public function rework()
    {
        ...
        foreach ($data as $key => $value) {
            foreach ($detail as $key => $value2) {
                // penjumlahan data tiap variable 
                $terpenuhi = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('target_terpenuhi');
                $total_check = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('total_check');
                $total_reject = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('total_reject');
                $fg = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('fg');
                $broken = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('broken');
                $skip = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('skip');
                $pktw = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('pktw');
                $crooked = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('crooked');
                $pleated = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('pleated');
                $ros = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('ros');
                $htl = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('htl');
                $button = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('button');
                $print = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('print');
                $bs = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('bs');
                $unb = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('unb');
                $shading = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('shading');
                $dof = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('dof');
                $dirty = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('dirty');
                $shiny = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('shiny');
                $sticker = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('sticker');
                $trimming = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('trimming');
                $ip = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('ip');
                $meas = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('meas');
                $other = LineDetail::where('tgl_pengerjaan', $tanggal)->get()->sum('other');
                if($terpenuhi == 0){
                    $p_fg = 0;
                    $p_broken = 0;
                    $p_skip = 0;
        ...
        // END
            ...
            return view('CommandCenter.allrework', compact('tanggal', 'totalSemuaLine', 'detailLine', 'mline','jum_wo'));
        }else{
    ...
    public function lines($id)
    {
        ...
        if(
            ...
            $jum_wo = collect($mline)->sum('jumlah_wo');
            // END
            return view('CommandCenter/alllines', compact('tanggal', 'dataBranch', 'px_total_reject', 'totalSemuaLine', 'detailLine', 'mline','jum_wo', 'id_line'));
        }else{
            throw new \Exception('Data Kosong !');
        }
    }
